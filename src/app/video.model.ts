...
export class Video {
  ...
  public videoRatings: number[];
  public videoRating: number = 0;
  public annotationRatings: number[];
  public annotationRating: number = 0;
  public videoCreated: string;
  public id: string;
  public dateCalculationsService: DateCalculationsService =
    new DateCalculationsService();
  public eventsAsCommaDelimitedString: string = null;
  // public eventsInVideo: Array<EventInVideo>;
  constructor(
    public videoDeets: VideoDetails,
  ...
  static fromJson(jsonObj: any): Video {
    ...
    let annotationRatings = jsonObj.annotationRatings
      ? jsonObj.annotationRatings
      : null;
    ...
    if (extractedEventsInVideo && videoDeets && originalPosterId) {
      ...
      let tmpMatch = new Video(
        videoDeets,
        originalPosterId,
        extractedEventsInVideo
      );
  ...
  updateEventsAsCommaDelimitedString(
    eventsInVideo: EventInVideo[],
    onlyGetSuccessful: boolean
  ) {
    ...
    if (onlyGetSuccessful) {
      ...
      eventNames = eventsInVideo
        .filter((x) => x.isSuccessfulAttempt == true)
        .map((x) => x.eventName);
