...
export class UserCollectionsDisplayComponent
  extends BaseComponent
  implements OnInit
{
  public collections: Collection[] = new Array<Collection>();
  ...
  ngOnInit() {
    this.trackerService.currentUserBehaviorSubject
      ...
      .subscribe((user) => {
        ...
        user
          ? (this.isAdmin = user.privileges.isAdmin)
          : (this.isAdmin = false);
  ...
  removeCollection(collection: Collection, user: any) {
    ...
    if (confirmation) {
      this.databaseService
        .deleteCollectionAndConfirm(collection, user)
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe((deletionStatus) => {
