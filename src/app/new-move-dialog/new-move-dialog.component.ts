...
export class NewMoveDialogComponent
  extends BaseDialogComponent
  implements OnInit
{
  form: FormGroup;
  public eventNameFc: FormControl = new FormControl("", [Validators.required]);
  public eventCategoryFc: FormControl = new FormControl("", [
    Validators.required,
  ]);
  public moveSubcategoryFc: FormControl = new FormControl("");
  public categories: any = constants.rootNodes.sort();
  public subcategories: any = constants.subCategories.sort();
  public localRootNodesWithSubcategories: Array<string> =
    constants.rootNodesWithSubcategories.sort();
  public displaySubcategorySelect: boolean = false;
  public displayCategoryName: boolean = true;

  constructor(
    public snackBar: MatSnackBar,
    public fb: FormBuilder,
    public dialogRef: MatDialogRef<NewMoveDialogComponent>,
    @Inject(MAT_DIALOG_DATA) { eventNameFc },
  ...
  getValues() {
    let move = this.textTransformationService.capitalizeFirstLetterOfEachWord(
      this.eventNameFc.value
    );
  ...
  processDialogData() {
    ...
    this.db
      .doesMoveNameAlreadyExistInDb(
        vals.move,
        vals.eventCategory,
        vals.moveSubcategory
      )
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((doesMoveNameAlreadyExistInDb) => {
  ...
  allValid() {
    ...
    if (this.displaySubcategorySelect) {
      if (
        this.vs.validateString(values.move) &&
        this.vs.validateString(values.eventCategory) &&
        this.vs.validateString(values.moveSubcategory)
      ) {
        return true;
      } else {
        return false;
