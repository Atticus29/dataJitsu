...
export class TournamentNameApprovalComponent
  extends BaseComponent
  implements OnInit
{
  public localCandidateTournamentNames: any = null;
  public localTournamentNames: any = null;
  ...
  approveTournamentName(tournamentMoveName: string) {
    let confirmation = confirm(
      "Are you sure you want to APPROVE the tournament name " +
        tournamentMoveName +
        "?"
    );
    if (confirmation) {
      ...
      if (this.localUser) {
        this.db.updateUserReputationPoints(
          this.localUser.id,
          constants.numberOfPointsToAwardForApprovingTournamentName,
          "You approved tournament name " + tournamentMoveName
        );
      }
    }
  }
  ...
  disapproveTournamentName(tournamentName: string) {
    let confirmation = confirm(
      "Are you sure you want to DISAPPROVE the tournament name " +
        tournamentName +
        "?"
    );
    if (confirmation) {
      this.db
        .getvideoUrlFromCandidateTournamentName(tournamentName)
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe((urlResult) => {
          this.db
            ...
            .subscribe((videoIdResult) => {
              this.db.updateTournamentNameInMatch(
                videoIdResult,
                constants.tournamentNameRemovedMessage
              );
            });
        });
      ...
      if (this.localUser) {
        this.db.updateUserReputationPoints(
          this.localUser.id,
          constants.numberOfPointsToAwardForApprovingTournamentName,
          "You disapproved tournament name " + tournamentName
        );
      }
    }
  }
