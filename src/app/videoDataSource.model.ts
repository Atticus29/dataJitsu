...
export class VideoDataSource extends MatTableDataSource<Video> {
  public matchesSubject = new BehaviorSubject<Video[]>([]);
  public loadingMatches = new BehaviorSubject<boolean>(false);
  ...
  filterPredicate = (data, filter: string) => {
    const accumulator = (currentTerm, key) => {
      return this.accumulateNestedObjectContentsInSearch(
        currentTerm,
        data,
        key
      );
  ...
  accumulateNestedObjectContentsInSearch(search, data, key) {
    ...
    if (typeof data[key] === "object") {
      for (const k in data[key]) {
        if (data[key][k] !== null) {
          search = this.accumulateNestedObjectContentsInSearch(
            search,
            data[key],
            k
          );
