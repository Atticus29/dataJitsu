...
export class StripeComponent extends BaseComponent implements OnInit {
  ...
  public localTitle = constants.title;
  public localPaymentStatus: any = false;
  public localUserNodeId: string = null;

  public stripe;
  public card;
  public loading = false;
  public confirmation;
  public subscriptionCost: number = constants.monthlyCost;
  ...
  ngOnInit() {
    combineLatest(this.authService.currentUserObservable, this.afAuth.authState)
      ...
      .subscribe((user) => {
        ...
        if (user) {
          if (user.id) {
            ...
            let pymnStats = this.trackerService.currentUserBehaviorSubject.pipe(
              switchMap((user) => this.dbService.hasUserPaid(user.id))
            );
            pymnStats
              .pipe(takeUntil(this.ngUnsubscribe))
  ...
  async handleForm(e) {
    ...
    if (error) {
      ...
      const subscriptionFun = this.functions.httpsCallable(
        "stripeCreateSubscription"
      );
      try {
        const res = await subscriptionFun({
          plan: constants.stipePlanId,
          source: source.id,
        }).toPromise();
        if (res) {
          this.subscriptionStatus =
            "Subscription is " +
            this.textTransformationService.capitalizeFirstLetter(res.status);
  ...
  async cancelSubscription(e) {
    let confirmation = confirm(
      "Are you sure you want to cancel your very affordable subscription to this great service?"
    );
    if (confirmation) {
      ...
      const subId = await this.dbService
        .getSubscriptionIdFromUser(this.localUserNodeId)
        .pipe(first())
        .toPromise(); //.subscribe(subId =>{  pipe(takeUntil(this.ngUnsubscribe)).
      // console.log(subId);
      const cancelSubscriptionFun = this.functions.httpsCallable(
