...
export class AnnotatedMovesDisplayComponent
  extends BaseComponent
  implements OnInit
{
  // public ngUnsubscribe: Subject<void> = new Subject<void>();
  public annotations: Array<any> = new Array<any>();
  public localFlagMin: number = 1000;
  public isAdmin: boolean = false;
  public videoId: string = null;
  public test: boolean = false;
  public localUserId: string = null;
  // public isFlaggedDirective: boolean = false;
  public isAdvantage: boolean = false;
  ...
  ngOnInit() {
    ...
    this.localFlagMin =
      constants.numberOfFlagsAnAnnotationNeedsBeforeItIsDisplayedToDrawAttention;
    let self = this;
    ...
    this.trackerService.fetchNewAnnotations
      .pipe(takeUntil(this.ngUnsubscribe))
  ...
  fetchAnnotations(videoId: string) {
    if (videoId) {
      ...
      this.databaseService
        .getAnnotationsSortedByStartTimeV2(
          videoId,
          "videos/" + videoId + "/events/"
        )
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe((annotationResults) => {
          //TODO ?
          if (annotationResults) {
            console.log(
              "annotationResults in fetchAnnotations call of AnnotatedMovesDisplayComponent:"
            );
  ...
  flagAnnotationForImprovement(
    ...
    if (this.localUserId) {
      ...
      this.databaseService.toggleAnnotationFlag(
        videoId,
        timeInitiated,
        this.localUserId
      );
      ...
      this.databaseService
        .getNumberOfUniqueAnnotationFlags(videoId, timeInitiated)
        .pipe(take(1))
        .subscribe((numberOfFlags) => {
          ...
          if (
            ...
            this.databaseService.updateUserReputationPoints(
              annotatorUserId,
              constants.numberOfPointsToDeductForBadAnnotation * -1,
              "Your annotation on match " +
                videoId +
                " was flagged too many times."
            );
          }
        });
    } else {
