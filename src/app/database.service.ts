...
export class DatabaseService {
  ...
  getEventIdByVideoIdAndStartTime(videoId: string, timeInitiated: number) {
    ...
    let obsRet = Observable.create(function (observer) {
      ref
        .orderByChild("timeInitiated")
        .equalTo(timeInitiated)
  ...
  deleteUserByEmail(emailAddress: string): any {
    return this.http
      .post<boolean>(deleteUserEndpointUrl, { userEmail: emailAddress })
      .pipe(
        catchError((res) => {
          return of(res.error);
        })
  ...
  getCandidateTournamentNames(): any {
    ...
    let obsRet = Observable.create(function (observer) {
      ref.orderByChild("name").on("value", (snapshot) => {
        ...
        if (resultObj) {
          let names = Object.keys(resultObj).map(
            (index) => resultObj[index].name
          );
          observer.next(names);
        } else {
          observer.next([]);
  ...
  getEventsSubsetAsObject(childNodeName: string) {
    ...
    let obsRet = Observable.create(function (observer) {
      if (constants.subCategories.indexOf(childNodeName) > -1) {
        ...
        ref
          .orderByChild("/Submissions or Submission Attempts/" + childNodeName)
          .on("value", (snapshot) => {
            ...
            observer.next(
              snapshot.val()["Submissions or Submission Attempts"][
                childNodeName
              ]
            );
          });
  ...
  getLowRatedVideo() {
    ...
    let obsRet = Observable.create(function (observer) {
      ref
        .orderByChild("/videoDeets/annotationRating")
        .limitToFirst(1)
        .on("child_added", (snapshot) => {
  ...
  getMovesKeys() {
    let ref = firebase.database().ref("events/");
    let textTransformationService: TextTransformationService =
      new TextTransformationService();
    // return ref;
    let obsRet = Observable.create(function (observer) {
      ref.orderByKey().on("value", (snapshot) => {
        let childKeys = [];
        snapshot.forEach(function (childSnapshot) {
          let category =
            textTransformationService.convertCamelCaseToSentenceCase(
              childSnapshot.key
            );
  ...
  getUsers(): Observable<User[]> {
    ...
    let resultObservable = Observable.create((observer) => {
      ref.on("value", (snapshot) => {
        const snapshotAsUsers = Object.values(snapshot.val()).map(
          User.fromJson
        );
  ...
  getUserReputationPoints(userId: string) {
    return this.db
      .object("users/" + userId + "/reputationPoints")
      .valueChanges();
  }

  updateUserReputationPoints(userId: string, points: number, reason: string) {
    let updates = {};
    this.getUserReputationPoints(userId)
      .pipe(take(1))
      .subscribe((result) => {
        //used to be first()
        ...
        // console.log(result);
        // console.log(points);
        updates["/users/" + userId + "/reputationPoints"] =
          Number(result) + points;
        firebase.database().ref().update(updates);
      });
    updates = {};
    let ref = this.db.list("/users/" + userId + "/reputationLog");
    let reputationLogEntry = new ReputationLog(
      new Date().toJSON(),
      reason,
      points
    );
  ...
  toggleAnnotationFlag(videoId: string, timeInitiated: number, userId: string) {
    this.getEventIdByVideoIdAndStartTime(videoId, timeInitiated)
      .pipe(take(1))
      .subscribe((annotationId) => {
        if (annotationId) {
          this.getAnnotationFlagStatus(videoId, annotationId, userId)
            .pipe(take(1))
            .subscribe((flaggedStatus) => {
              if (flaggedStatus) {
  ...
  updateNumFlags(videoId: string, annotationId: string, points: number) {
    ...
    let ref = firebase
      .database()
      .ref("videos/" + videoId + "/events/" + annotationId + "/numFlags");
    ref.once("value", (snapshot) => {
      // console.log("updateNumFlags current points snapshot:");
      let currentVal: number = snapshot.val();
      // console.log(currentVal);
      updates["/videos/" + videoId + "/events/" + annotationId + "/numFlags"] =
        currentVal + points;
      firebase.database().ref().update(updates);
    });
  }

  getAnnotationFlagStatus(
    videoId: string,
    annotationId: string,
    userId: string
  ) {
    let queryObservable = Observable.create(function (observer) {
      let ref = firebase
        .database()
        .ref(
          "videos/" + videoId + "/events/" + annotationId + "/flags/" + userId
        );
      ref.once("value", (snapshot) => {
  ...
  getNumberOfUniqueAnnotationFlags(videoId: string, timeInitiated: number) {
    ...
    let queryObservable = Observable.create(function (observer) {
      if (videoId && timeInitiated) {
        self
          .getEventIdByVideoIdAndStartTime(videoId, timeInitiated)
          .pipe(take(1))
          .subscribe((annotationId) => {
            if (annotationId) {
              let ref = firebase
                .database()
                .ref(
                  "videos/" + videoId + "/events/" + annotationId + "/numFlags"
                );
              ref.on("value", (snapshot) => {
  ...
  getMatchCount() {
    //TODO this is very inefficient. Some not-great leads here: https://stackoverflow.com/questions/15148803/in-firebase-is-there-a-way-to-get-the-number-of-children-of-a-node-without-load
    let ref = firebase.database().ref("videos/");
    let queryObservable = Observable.create(function (observer) {
      ref.on("value", (snapshot) => {
  ...
  addVideoToDb(video: any) {
    ...
    updates["/videos/" + videoId + "/videoCreated"] =
  ...
  addVideoToDbWithPath(video: any, path: string): Observable<string> {
    ...
    updates[path + videoId + "/videoCreated"] =
      firebase.database.ServerValue.TIMESTAMP;
    updates[
      "users/" +
        video.videoDeets.genericArgs.originalPosterId +
        "/" +
        path +
        videoId +
        "/"
  ...
  addEventInVideoToVideoIfUniqueEnough(
    move: EventInVideo
  ): Observable<boolean> {
    ...
    let resultObservable = Observable.create((observer) => {
      ...
      this.eventIsUniqueEnough(move, "videos/" + move.getVideoId() + "/events/")
        .pipe(takeUntil(localUnsubscribeSubject))
        .subscribe((uniqueEnough) => {
  ...
  eventIsUniqueEnough(move: EventInVideo, path: string): Observable<boolean> {
    ...
    let resultObservable = Observable.create((observer) => {
      ...
      this.getAnnotations(move.getVideoId(), path)
        .pipe(takeUntil(localUnsubscribeSubject))
        .subscribe((events) => {
          ...
          if (events) {
            for (let item in events) {
              ...
              if (
                events[item].eventName === move.eventName &&
                events[item].actor === move.actor &&
                this.annotationWithinTimeRange(events[item], move)
              ) {
                //TODO and start time is within 2 seconds of start time and same with end time
                observer.next(false);
                localUnsubscribeSubject.next();
                localUnsubscribeSubject.complete();
                return resultObservable;
  ...
  annotationWithinTimeRange(
    oldMove: EventInVideo,
    candidateNewMove: EventInVideo
  ): boolean {
    let candidateNewMoveDoesNotStartTooEarly =
      oldMove.timeInitiated -
        constants.numberOfSecondsToleratedToBeCalledSameAnnotation <=
      candidateNewMove.timeInitiated;
    let candidateNewMoveDoesNotStartTooLate =
      oldMove.timeInitiated +
        constants.numberOfSecondsToleratedToBeCalledSameAnnotation >=
      candidateNewMove.timeInitiated;
    let candidateNewMoveDoesNotEndTooEarly =
      oldMove.timeCompleted -
        constants.numberOfSecondsToleratedToBeCalledSameAnnotation <=
      candidateNewMove.timeCompleted;
    let candidateNewMoveDoesNotEndTooLate =
      oldMove.timeCompleted +
        constants.numberOfSecondsToleratedToBeCalledSameAnnotation >=
      candidateNewMove.timeCompleted;
    return (
      candidateNewMoveDoesNotStartTooEarly &&
      candidateNewMoveDoesNotStartTooLate &&
      candidateNewMoveDoesNotEndTooEarly &&
      candidateNewMoveDoesNotEndTooLate
    );
  }
      ...
      ref.orderByChild("timeInitiated").on("child_added", (snapshot) => {
        ...
        if (snapshot) {
          if (snapshot.val()) {
            ...
            let currentEventInVideo = new EventInVideo(
              snapshotVals.eventName,
              snapshotVals.eventCategory,
              snapshotVals.actor,
              snapshotVals.recipient,
              snapshotVals.timeInitiated,
              snapshotVals.timeCompleted,
              snapshotVals.points,
              snapshotVals.associatedVideoId,
              snapshotVals.isASubmission,
              snapshotVals.isSuccessfulAttempt,
              snapshotVals.annotatorUserId
            );
            ...
            currentEventInVideo.setIsVideoActionDelimiter(
              snapshotVals.isMatchActionDelimiter
            );
  ...
  addEventInVideoToUserIfUniqueEnough(
    move: EventInVideo,
    currentUserId: string
  ): Observable<boolean> {
    console.log(
      "addEventInVideoToUserIfUniqueEnough called in database service"
    );
    // debugger;
    let localUnsubscribeSubject: Subject<void> = new Subject<void>();
    let resultObservable = Observable.create((observer) => {
      ...
      if (move.getMoveName() !== "No Annotation Currently Selected") {
        this.eventIsUniqueEnough(
          move,
          "users/" + currentUserId + "/eventsAnnotated/"
        )
          .pipe(takeUntil(localUnsubscribeSubject))
          .subscribe((uniqueEnough) => {
            if (uniqueEnough && counter < 1) {
              ...
              let videoId = move.getVideoId();
              let ref = this.db.list(
                "/users/" + currentUserId + "/eventsAnnotated"
              );
              let moveId = ref.push(move).key;
              let updates = {};
              updates[
                "/users/" + currentUserId + "/eventsAnnotated/" + moveId
              ] = move;
              // console.log(updates);
              firebase.database().ref().update(updates);
              //Now update dateLastAnnotated
              ref = this.db.list(
                "/users/" + currentUserId + "/dateLastAnnotated"
              );
              updates = {};
  ...
  userHasAnnotatedEnough(userId: string): Observable<boolean> {
    ...
    let resultObservable = Observable.create((observer) => {
      let ref = firebase
        .database()
        .ref("users/" + userId + "/annotatedEnoughOverride/");
      ref.on("value", (status) => {
        ...
        if (!skipTheActualCalculationBecauseOfOverride) {
          ref = firebase
            .database()
            .ref("users/" + userId + "/eventsAnnotated/");
          let moveCount = 0;
          ref.on("child_added", (childSnapshot) => {
            ...
            if (
              this.dateCalculationsService.calculateDaysSinceLastAnnotation(
                new Date(move.dateAdded)
              ) <= constants.numDaysBeforeNewAnnotationNeeded
            ) {
  ...
  addMatchRatingToUser(userId: string, videoId: string, rating: number) {
    ...
    let updates = {};
    updates["/users/" + userId + "/videosRated/" + videoId + "/videoRating/"] =
      rating;
    firebase.database().ref().update(updates);
  }

  addMatchAnnotationRatingToUser(
    userId: string,
    videoId: string,
    ...
    let updates = {};
    updates[
      "/users/" + userId + "/videosRated/" + videoId + "/annotationRating/"
    ] = annotationRating;
    firebase.database().ref().update(updates);
  }
  ...
  addMatchAnnotationRatingToMatch(
    userId: string,
    videoId: string,
    annotationRating: number
  ) {
    let updates = {};
    updates["/videos/" + videoId + "/annotationRatings/" + userId] =
      annotationRating;
    firebase.database().ref().update(updates);
  }

  average(list: any[]) {
    return this.dateCalculationsService.roundToDecimal(
  ...
  getAverageAnnotationRating(videoId: string) {
    let ref = firebase
      .database()
      .ref("videos/" + videoId + "/annotationRatings/");
    let resultObservable = Observable.create((observer) => {
      ref.on("value", (snapshot) => {
        let results = snapshot.val();
        if (results) {
  ...
  updateAnnotationRating(videoId: string, annotationRating: number) {
    let updates = {};
    updates["videos/" + videoId + "/videoDeets/annotationRating"] =
      annotationRating;
    firebase.database().ref().update(updates);
  }
  ...
  doesMatchExist(videoUrl: string) {
    ...
    let resultObservable = Observable.create((observer) => {
      ref
        .orderByChild("/videoDeets/videoUrl")
        .equalTo(videoUrl)
        .limitToFirst(1)
        .once("value", (snapshot) => {
          // entranceDetector = 1;
  ...
  isAdmin(userId: string): Observable<boolean> {
    let ref = firebase
      .database()
      .ref("users/" + userId + "/privileges/isAdmin");
    let resultObservable = Observable.create((observer) => {
      ref.on("value", (snapshot) => {
        if (snapshot.val()) {
          observer.next(true);
  ...
  removeAnnotationInMatchAndUserByStartTime(
    videoId: string,
    ...
    let resultObservable = Observable.create((observer) => {
      ...
      let newRef = firebase
        .database()
        .ref("users/" + annotatorUserId + "/eventsAnnotated/");
      newRef
        .orderByChild("timeInitiated")
        .equalTo(timeInitiated)
        .on("child_added", (snapshot) => {
  ...
  getVideoRemovedFlagStatus(videoId: string) {
    let resultObservable = Observable.create((observer) => {
      let ref = firebase
        .database()
        .ref("/videos/" + videoId + "/videoDeets/flaggedAsMissing");
      ref.on("value", (snapshot) => {
        if (snapshot.val()) {
  ...
  getInappropriateFlagStatus(videoId: string) {
    let resultObservable = Observable.create((observer) => {
      let ref = firebase
        .database()
        .ref("/videos/" + videoId + "/videoDeets/flaggedAsInappropriate");
      ref.on("value", (snapshot) => {
        if (snapshot.val()) {
  ...
  flagVideoInappropriateInMatch(videoId: string, status: boolean) {
    let updates = {};
    updates["/videos/" + videoId + "/videoDeets/flaggedAsInappropriate"] =
      status;
    firebase.database().ref().update(updates);
  }
  ...
  addCandidateNameToDb(name: string, associatedvideoUrl: string) {
    ...
    let keyId = ref.push({
      name: name,
      associatedvideoUrl: associatedvideoUrl,
    }); //.key;
  }
  ...
  addCandidateTournamentNameToDb(name: string, associatedvideoUrl: string) {
    ...
    this.addGenericCandidateNameToDb(
  ...
  addGenericCandidateNameToDb(
    ...
    if (this.validationService.validateUrl(identifyingInformation)) {
      ref
        .push()
        .set({ name: name, associatedvideoUrl: identifyingInformation }); //.key;
    } else {
  ...
  updateAthleteNameInMatch(
    ...
    let ref = firebase.database().ref("/videos/");
    ref
      .orderByChild("videoDeets/athlete1Name")
      .equalTo(targetName)
      .on("child_added", (snapshot) => {
    ...
    ref
      .orderByChild("videoDeets/athlete2Name")
      .equalTo(targetName)
      .on("child_added", (snapshot) => {
  ...
  addCandidateEventInVideoToDb(
    ...
    //TODO associatedvideoUrl
    this.addGenericItemToDb("/candidateMoveNames/", {
      eventName: eventName,
  ...
  addMoveNameToDb(
    ...
    // ref.push(eventName);
    this.addGenericItemToDb(
      "/events/" + categoryName + "/" + subcategoryName + "/",
      eventName
    );
  }
        ...
        ref
          ...
          .on("child_added", (snapshot) => {
            ...
            if (Array.isArray(snapshot.val()[subcategoryName])) {
              ...
              if (
                typeof snapshot.val()[subcategoryName] === "object" &&
                snapshot.val()[subcategoryName] !== null
              ) {
                if (
                  Object.values(snapshot.val()[subcategoryName]).indexOf(
                    eventName
                  ) > -1
                ) {
                  observer.next(true);
                ...
                console.log(
                  "Congrats you found a bug I never expected in my doesMoveNameAlreadyExistInDb. Please report exactly what kind of move you were trying to make!"
                );
                alert(
                  "Congrats you found a bug I never expected. Please report!"
                );
              }
            }
          });
  ...
  removeMoveNameFromCandidateList(eventName: string) {
    this.removeGenericStringWithOrderByFromDb(
      "/candidateMoveNames/",
      "eventName",
      eventName
    );
  ...
  removeGenericStringWithOrderByFromDb(
    path: string,
    orderIndex: string,
    ...
    let ref = firebase.database().ref(path);
    ref
      .orderByChild(orderIndex)
      .equalTo(genericString)
      .on("child_added", (snapshot) => {
        ref.child(snapshot.key).remove();
      });
  }
  ...
  getCandidateMoveNames() {
    ...
    let obsRet = Observable.create(function (observer) {
      ref.orderByChild("eventName").on("value", (snapshot) => {
        ...
        if (resultObj) {
          let names = Object.keys(resultObj).map(
            (index) => resultObj[index].eventName
          );
          observer.next(names);
        } else {
          observer.next([]);
  ...
  getvideoUrlFromMatchId(videoId: string) {
    let ref = firebase
      .database()
      .ref("/videos/" + videoId + "/videoDeets/videoUrl");
    let obsRet = Observable.create(function (observer) {
      ref.orderByKey().on("value", (snapshot) => {
  ...
  getvideoUrlFromCandidateTournamentName(tournamentName: string) {
    ...
    // });
    // return obsRet;
    return this.getIdentifyingInformationFromGenericCandidateName(
      "/candidateTournamentNames/",
      "name",
      tournamentName
    );
  ...
  updateMoveNameInMatch(
    ...
    // firebase.database().ref().update(updates);
    this.updateGenericNameInVideo(
      "/events/" + moveId + "/eventName/",
      videoId,
      newName
    );
  }
  ...
  deleteMoveName(eventName: string, categoryName: string, subcategory: string) {
    //just put '' if there is no subcategory
    subcategory = subcategory + "/";
    let ref = firebase
      .database()
      .ref("events/" + categoryName + "/" + subcategory);
    ref
      .orderByValue()
      .equalTo(eventName)
      .on("child_added", (snapshot) => {
  ...
  getSubcategoryFromMoveAndCategory(category: string, move: string) {
    ...
    let obsRet = Observable.create(function (observer) {
      ref.orderByKey().on("value", (snapshot) => {
        ...
        let arregateArray = Object.keys(categoryObj).map(
          (subcat) => categoryObj[subcat]
        );
  ...
  getSubscriptionIdFromUser(userNodeId: string) {
    let ref = firebase
      .database()
      .ref("/users/" + userNodeId + "/subscriptionInfo/subscriptionId");
    let obsRet = Observable.create(function (observer) {
  ...
  doesGenricCandidateAlreadyExistInDb(
    path: string,
    name: string
  ): Observable<boolean> {
        ...
        ref.orderByKey().on("value", (snapshot) => {
          if (snapshot.val()) {
            ...
            collections.forEach((dbCollection) => {
              let currentDbCollection: Collection =
                Collection.fromDataBase(dbCollection);
  ...
  addCollectionToDatabase(
    ...
    let obsRet = Observable.create(function (observer) {
      self
        .doesCollectionAlreadyExistInDb(collection)
        .pipe(take(1))
        .subscribe((alreadyExists) => {
          if (alreadyExists) {
            ...
            let updates = {};
            updates["/users/" + userId + "/collections/" + collectionId] =
              collection;
  ...
  addOwnerQuestionSetToDatabase(
    ownerQuestionSet: OwnerQuestionSet,
    userId: string
  ): Observable<boolean> {
    ...
    let obsRet = Observable.create(function (observer) {
      ...
      let updates = {};
      updates[
        "/users/" + userId + "/collections/" + collectionId + "/ownerQuestions/"
      ] = ownerQuestionSet.getOwnerQuestions();
      updates["/collections/" + collectionId + "/ownerQuestions/"] =
        ownerQuestionSet.getOwnerQuestions();
      firebase.database().ref().update(updates);
      observer.next(true);
      return obsRet;
  ...
  updateUser(
    questionAssociatedWithUpdateVal: any,
    path: string,
    updateVal: string
  ): Observable<boolean> {
    let self = this;
    let obsRet = Observable.create(function (observer) {
      ...
      if (questionAssociatedWithUpdateVal.pathToConfirmedValues) {
        self
          .doesGenricCandidateAlreadyExistInDb(
            questionAssociatedWithUpdateVal.pathToConfirmedValues,
            updateVal
          )
          .pipe(take(1))
          .subscribe((alreadyExists) => {
            ...
            if (!alreadyExists) {
              self.addGenericCandidateNameToDb(
                questionAssociatedWithUpdateVal.pathToCandidateValues,
                self.textTransformationService.capitalizeFirstLetter(updateVal),
                path
              );
            }
          });
  ...
  updateVideoDeet(
    ...
    let obsRet = Observable.create(function (observer) {
      let updates = {};
      if (
        path.indexOf(constants.nameOfIndividual1InDb) > -1 ||
        path.indexOf(constants.nameOfIndividual2InDb) > -1
      ) {
        let subpath = path.substring(
          0,
          path.indexOf(constants.nameOfVideoDetailsInDb)
        );
        ...
        events.once("value", (snapshot) => {
          snapshot.forEach((childSnapshot) => {
            ...
            if (childData.actor === oldVal) {
              ...
              updates[eventsPath + "/" + childKey + "/" + actorOrRecipient] =
                updateVal;
            }
            if (
              childData.actor === constants.noneEntered &&
              path.indexOf(constants.nameOfIndividual1InDb) > -1
            ) {
            ...
            if (
              childData.actor === constants.noneEntered2 &&
              path.indexOf(constants.nameOfIndividual2InDb) > -1
            ) {
            ...
            if (
              childData.recipient === constants.noneEntered &&
              path.indexOf(constants.nameOfIndividual1InDb) > -1
            ) {
            ...
            if (
              childData.recipient === constants.noneEntered2 &&
              path.indexOf(constants.nameOfIndividual2InDb) > -1
            ) {
      ...
      console.log(
        "questionAssociatedWithUpdateVal.enableAddNew is: " +
          questionAssociatedWithUpdateVal.enableAddNew
      );
    ...
    let obsRet = Observable.create(function (observer) {
      ...
      if (user) {
        if (user.id) {
          ...
          ref = firebase
            .database()
            .ref("users/" + user.id + "/collections/" + collection.getId());
          ref.remove();
          observer.next(true);
        }
      }
    ...
    let obsRet = Observable.create(function (observer) {
      ref.once("value").then((snapshot) => {
        ...
        if (snapshot.val()) {
          ...
          let feedbackItemArray = feedbackObjArray.map(
            FeedbackItem.fromDataBase
          );
