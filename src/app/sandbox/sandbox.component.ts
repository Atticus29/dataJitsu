...
export class SandboxComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.questionService.getTestQuestions().pipe(takeUntil(this.ngUnsubscribe)).subscribe(questionResults =>{
      this.localTestQuestionConfigOptions = new DynamicFormConfiguration(questionResults, [], "Done");
