...
export class UserDeleteComponent extends BaseComponent implements OnInit {
    ...
    this.formProcessingService.formSubmitted
      ...
      .subscribe((isFormSubmitted) => {
        ...
        if (isFormSubmitted && this.stopCounter < 1) {
          ...
          this.stopCounter++;
          const formResultObservableWithLatestQuestions =
  ...
  handleUserDeletion(formResults, self) {
    ...
    this.databaseService
      .getFirstUserByUsername(userName)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((results) => {
        try {
          const deletionResponse: Observable<any> =
            this.databaseService.deleteUserByEmail(results.email);
          deletionResponse
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe((result) => {
              ...
              if (!result.ok) {
                ...
                this.databaseService
                  .deleteUserFromDatabase(results.email)
                  .pipe(takeUntil(this.ngUnsubscribe))
                  .subscribe((wasDbDeleteSuccessful) => {
