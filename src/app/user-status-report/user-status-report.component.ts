...
export class UserStatusReportComponent extends BaseComponent implements OnInit {
  ...
  public notificationsSeen: boolean = false;
  ...
  ngOnInit() {
    try {
      let pymnStats = this.trackerService.currentUserBehaviorSubject.pipe(
        switchMap((user) =>
          user ? this.dbService.hasUserPaid(user.id) : of(false)
        )
      );
      pymnStats
          ...
          if (this.user.uid) {
            this.dbService
              ...
              .subscribe((dbUser) => {
                ...
                this.dbService
                  .userHasAnnotatedEnough(this.userObjFromDb.id)
                  .pipe(takeUntil(this.ngUnsubscribe))
                  .subscribe((hasUserAnnotatedEnough) => {
