...
export class AthleteNameApprovalComponent
  extends BaseComponent
  implements OnInit
{
  public localCandidateAthleteNames: any = null;
  public localAtheleteNames: any = null;
  ...
  approveName(name: string) {
    ...
    let confirmation = confirm(
      "Are you sure you want to APPROVE the name " + name + "?"
    );
    if (confirmation) {
      ...
      if (this.localUser) {
        this.db.updateUserReputationPoints(
          this.localUser.id,
          constants.numberOfPointsToAwardForApprovingCandidateAthleteName,
          "You approved candidate athlete name " + name
        );
      }
    }
  }
  ...
  disapproveName(name: string) {
    ...
    let confirmation = confirm(
      "Are you sure you want to DISAPPROVE the name " + name + "?"
    );
    if (confirmation) {
      this.db
        .getvideoUrlFromCandidateAthleteName(name)
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe((urlResult) => {
          ...
          this.db
            ...
            .subscribe((videoIdResult) => {
              this.db.updateAthleteNameInMatch(
                videoIdResult,
                name,
                "Un-named Athlete"
              );
      ...
      if (this.localUser) {
        this.db.updateUserReputationPoints(
          this.localUser.id,
          constants.numberOfPointsToAwardForApprovingCandidateAthleteName,
          "You disapproved candidate athlete name " + name
        );
      }
    }
  }
