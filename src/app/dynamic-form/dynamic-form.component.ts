  ...
  implements OnInit, OnDestroy
{
  ...
  form: FormGroup;
  payLoad = "";
  public localButtonDisplayName: String;
  checked = false;
  public nonRequiredIsInvalid = true;
  public totalNumberOfQuestions = 0;
  public uniqueQuestionKeys: string[] = [];
  public confirmMatchesOtherQuestionVal = true;
  ...
  ngOnInit() {
    ...
    if (this.questions) {
      ...
      if (this.questions.length > -1) {
        ...
        const requireds = this.questions
          .map((question) => question.required)
    ...
    this.formProcessingService.questionArrayOfForm
      ...
      .subscribe((questionArrayOfForm) => {
        ...
        if (!this.form) {
          ...
          self.formProcessingService.buttonDisplayName
            .pipe(takeUntil(self.ngUnsubscribe))
            .subscribe((buttonDisplayName) => {
  ...
  addAnotherQuestion(
    question: FormQuestionBase<string>,
    questionArray: FormQuestionBase<string>[],
    index: number,
    submitAfterThisQuestion: boolean
  ) {
    let newQuestionToBeAdded: FormQuestionBase<string> =
      FormQuestionBase.makeNewQuestionWithGiveOptionToAnswerThisQuestionMultipleTimesAs(
        question,
        true,
        true,
        submitAfterThisQuestion
      ); //TODO last argument true??
    let baseKey: string = question.key.split(/\d+/)[0];
    let newIndex: number =
      FormQuestionBase.calculateCurrentHighestIndexWithThisBaseKey(
        baseKey,
        questionArray
      ) + 1;
    newQuestionToBeAdded =
      FormQuestionBase.createNewQuestionModifyingKeyOfExistingQuestion(
        newQuestionToBeAdded,
        baseKey + newIndex
      );
    let previousQuestionModified =
      FormQuestionBase.makeNewQuestionWithGiveOptionToAnswerThisQuestionMultipleTimesAs(
        question,
        false,
        false,
    ...
    previousQuestionModified =
      FormQuestionBase.createNewQuestionByModifyingExistingQuestion(
        previousQuestionModified,
        false,
        false
      );
    questionArray[index] = previousQuestionModified;
    let questionArrayCombiningNewAndOld =
      FormQuestionBase.spliceWithoutManipulatingOriginal(
        questionArray,
        [newQuestionToBeAdded],
        index
      );
    ...
    this.formProcessingService.captureQuestionArrayOfCurrentForm(
      questionArrayCombiningNewAndOld
    );
    this.form = this.qcs.toFormGroup(questionArrayCombiningNewAndOld);
    this.formProcessingService.actualForm.next(this.form);
    this.repopulateFormWithPreviousPayload(
      this.form,
      objectPayLoad,
      questionArrayCombiningNewAndOld
    );
  }
  addAnotherQuestionGroup(
    question: FormQuestionBase<string>,
    questionArray: FormQuestionBase<string>[],
    index: number
  ) {
    let lastSiblingIndex = question.findLastSiblingQuestionIndex(
      question,
      questionArray,
      index
    );
    let updatedQuestion: FormQuestionBase<string> =
      FormQuestionBase.createNewQuestionByModifyingExistingQuestion(
        question,
        false,
        false
    ...
    let renamedNewQuestionGroup = FormQuestionBase.renameNewQuestionGroup(
      questionArray,
      newQuestionGroup
    );
    let questionArrayCombiningNewAndOld =
  ...
  repopulateFormWithPreviousPayload(
    form: FormGroup,
    payLoad: Object,
    questionArray: FormQuestionBase<string>[]
  ) {
    ...
    for (let i = 0; i < payLoadKeys.length; i++) {
      if (questionArray.findIndex((q) => q.key === payLoadKeys[i]) > -1) {
        let correspondingQuestionIndex = questionArray.findIndex(
          (q) => q.key === payLoadKeys[i]
        );
        let populatedFormControl: any =
          questionArray[correspondingQuestionIndex] &&
          questionArray[correspondingQuestionIndex].required
            ? new FormControl(payLoadValues[i] || "", Validators.required)
  ...
  proccessIsFormQuestionValid(isValidFormEmittedData: any) {
    if (this.questions) {
      let questionKeys = this.questions.map(
        (currentQuestion) => currentQuestion.key
      );
      let focalQuestionIndex = questionKeys.indexOf(
        isValidFormEmittedData.questionKey
      );
      ...
      if (focalQuestion.keyOfOtherQuestionWhereMatchIsDesired) {
        let focalQuestionVal =
          this.form.controls[isValidFormEmittedData.questionKey].value;
        let referencedQuestion =
          this.form.controls[
            focalQuestion.keyOfOtherQuestionWhereMatchIsDesired
          ];
      ...
      let isRequired = this.questions[focalQuestionIndex].required
        ? this.questions[focalQuestionIndex].required
        : false;
      if (
        this.questions &&
        isRequired &&
        !this.uniqueQuestionKeys.includes(isValidFormEmittedData.questionKey) &&
        isValidFormEmittedData.isValid
      ) {
      ...
      if (this.questions && isRequired && !isValidFormEmittedData.isValid) {
        //not valid
        let uniqueQuestionKeysIndex = this.uniqueQuestionKeys.indexOf(
          isValidFormEmittedData.questionKey
        );
        if (uniqueQuestionKeysIndex > -1) {
          //removing a question from the validated list
