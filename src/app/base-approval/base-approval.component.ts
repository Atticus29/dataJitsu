...
export class BaseApprovalComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.db
      .getGenericApprovedList(this.approvalConfig.localApprovedListPath)
    ...
    this.db
      .getGenericAndOrderBy(this.approvalConfig.candidatePath, "name")
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((results) => {
  ...
  approveName(name: string, metaDataName: string) {
    ...
    if (confirmation) {
      this.db.addGenericItemToDb(
        this.approvalConfig.localApprovedListPath,
        name
      );
      this.db.removeGenericStringWithOrderByFromDb(
        this.approvalConfig.candidatePath,
        "name",
        name
      );
      if (this.localUser) {
        this.db.updateUserReputationPoints(
          this.localUser.id,
          this.approvalConfig.localApprovalAndDisapprovalPoints,
  ...
  disapproveName(name: string, metaDataName: string) {
    ...
    if (confirmation) {
      this.db
        ...
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe((identifyingInfoResult) => {
          if (identifyingInfoResult) {
            if (
              this.approvalConfig.localSubPathToMatchParameter.includes(
                constants.videoDetails
              )
            ) {
              this.db
                .getVideoIdFromVideoUrl(identifyingInfoResult)
                .pipe(takeUntil(this.ngUnsubscribe))
                .subscribe((videoIdResult) => {
                  console.log(
                    "getVideoIdFromVideoUrl from disapproveName is " +
                      videoIdResult
                  );
                  this.db.updateGenericNameInVideo(
                    this.approvalConfig.localSubPathToMatchParameter,
                    videoIdResult,
                    this.approvalConfig.localReplacementText
                  );
                });
            }
            if (
              ...
              this.db.updateGenericNameInUser(
                this.approvalConfig.localSubPathToMatchParameter,
                identifyingInfoResult,
                this.approvalConfig.localReplacementText
              );
            }
          }
