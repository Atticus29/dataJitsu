...
export class UserInfoEditComponent extends BaseComponent implements OnInit {
  public displayItems: {}[] = [];
  public editableParams: String[] = null;
  ...
  public userInDbId: any = null;
  public localPrivileges: any = null;
  ...
  ngOnInit() {
    ...
    let localUsr = this.trackerService.currentUserBehaviorSubject
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((usrResults) => {
        if (usrResults) {
            ...
            if (constants.displayableUserInfoObj[param] != null) {
              ...
              if (param === constants.privilegesParamName) {
                ...
                paramObjKeys.forEach((paramObjkey) => {
                  this.localPrivileges.push({
                    displayName:
                      constants.displayablePrivilegesInfoObj[paramObjkey],
                    val: paramObj[paramObjkey],
                  });
                });
              ...
              if (
                typeof usrResults[param] === "string" &&
                param.includes(constants.stringSharedByAllDateParams)
              ) {
                let transformedText =
                  this.textTransformationService.simplifyDateString(
                    usrResults[param]
                  );
                currentObj["param"] = transformedText;
              }
              currentObj["displayWords"] =
                constants.displayableUserInfoObj[param];
              // console.log("currentObj is: ");
              // console.log(currentObj);
              // console.log("type is: ");
              // console.log(typeof usrResults[param]);
              this.displayItems = this.replaceOrAddInto(
                this.displayItems,
                currentObj
              );
            }
  ...
  replaceOrAddInto(arrayOfItems, currentCandidate) {
    ...
    let indexOfMatch = arrayOfItems
      .map((item) => item.originalKey)
      .indexOf(currentCandidate.originalKey);
    if (indexOfMatch < 0) {
  ...
  toggleEdit(displayItem: any) {
    ...
    this.questionService
      ...
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((userEditQuestionsObj) => {
        //TODO change this method to one that fetches a hashmap of question objects keyed by displayItem.originalKey and pull it out by said key
        ...
        if (userEditQuestionsObj && originalKey) {
          this.localConfigOptions = new DynamicFormConfiguration(
            userEditQuestionsObj[originalKey],
            [],
            "Save"
          );
                  ...
                  if (currentFormQuestions) {
                    if (currentFormQuestions[0] !== "Stop") {
                      if (this.userInDbId) {
                        ...
                        if (path && updateVal) {
                          console.log(
                            "currentFormQuestions right before entering updateUser is:"
                          );
                          ...
                          this.databaseService
                            ...
                            .subscribe((additionStatus) => {
                              ...
                              if (additionStatus) {
                                self.openSnackBar(
                                  constants.userDetailUpdatedNotification
                                );
                                ...
                                self.openSnackBar(
                                  constants.userDetailUpdateFailureNotification
                                );
                                this.hideEditAndShowNewValue(formResults);
                                // self.displayModeInd1 = !self.displayModeInd1;
                              }
